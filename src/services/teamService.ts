...
export const createTeam = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!v.isValidTeamName(name)) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, data: null, meta: null, error: "팀 이름은 2자 이상 30자 이하로 입력해주세요." });
    }
    if (!v.isValidPin(pin_password)) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, data: null, meta: null, error: "핀 번호는 6자리 숫자여야 합니다." });
    }
    // name 중복 확인 
...
export const joinTeam = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const member = await findTeamMember(teamId, userId);
    if(member) {
        return res.status(StatusCodes.OK).json(SUCCESS({ message: "성공적으로 처리되었습니다." }))
    }

    // 신규 가입 시 PIN 번호 검증
    if (!v.isValidPin(password)) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, data: null, meta: null, error: "올바른 6자리 핀 번호를 입력해주세요." });
    }
    ...
    res.status(StatusCodes.CREATED).json(SUCCESS({ message: "성공적으로 처리되었습니다." }));
});
